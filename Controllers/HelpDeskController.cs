...
public class HelpDeskController : ControllerBase
{
    ...
    [HttpPost("flabo_help_desk")]
    public async Task<IActionResult> GetInvestigations([FromBody] HelpDeskRequestDto request)
    {
        try
        {
            if (!ModelState.IsValid)
            ...
            using (SqlConnection conn = new SqlConnection(connString))
            using (SqlCommand cmd = new SqlCommand("S_FlaboGetPatientInvestigationForLabHelpDesk", conn))
            {
                cmd.CommandType = CommandType.StoredProcedure;
